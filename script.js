...
function viewPresets() {
    ...
    if (presets.length === 0) {
        presetContainer.appendChild(warningP);
    } else {
        renderPresets();
    }
...
function addChoicesClick() {
    if (textInput.value === '') {
        changeBeatoImgText(
            'assets/beato-angry.webp',
            `You can't just leave the space blank, ${userName}! You need to add something!`
        );
        return;
    }
...
function decideChoices() {
    if (choiceList.length === 0) {
        changeBeatoImgText(
            'assets/beato-super-angry.webp',
            `Are you trying to make me angry, ${userName}? There's nothing on the list!`
        );
        return;
    }
    ...
    if (choiceList.length === 1) {
        changeBeatoImgText(
            'assets/beato-confused.webp',
            'Ha? Why would you want me to choose only one thing?'
        );
        return;
    }
    ...
    changeBeatoImgText(
        'assets/beato-choose.webp',
        `Ahaha! My choice is: "${choiceList[beatosChoice]}"`
    );
    ahaha.volume = 0.3;
    ahaha.play();
}
...
function getName() {
    ...
    if (nameValue === '') {
        nameP.textContent = "Enter something, furniture! You don't want to make me angry!";
